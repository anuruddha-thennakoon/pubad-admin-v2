...
@inject('appStore')
@observer
class GradeIIOfficers extends React.Component {

    constructor(props) {
        super(props);
        this.state = { loading: true, filter: 'Appointed Officers in Grade II Posts' };
    ...
    filterData = (data) => {
        ...
        if (filter == 'Appointed Officers in Grade II Posts') {
            var temp = [];
            data.forEach(element => {
                if (element.officer_grade == 3 && element.nature_of_attachment == 'Permanent' && element.position_grade == 4) {
                    temp.push(element)
                }
            });
            return temp;
            ...
            var temp = [];
            data.forEach(element => {
                if (element.officer_grade == 3 && element.position_grade == 7) {
                    temp.push(element)
                }
            });
            return temp;
            ...
            var temp = [];
            data.forEach(element => {
                if (element.officer_grade == 3 && element.position_grade == 6) {
                    temp.push(element)
                }
            });
            return temp;
        ...
        return (
            <div>
                <Card bordered={false}>
                    ...
                    <Title level={4}>Grade II Officers</Title>
                ...
                <Card className="card-magrin">
                    <div>
                        <Text>Type : </Text>
                        <Select
                            defaultValue="Appointed Officers in Grade II Posts"
                            style={{ width: 400 }}
                        ...
                        >
                            <Option value="Appointed Officers in Grade II Posts">Appointed Officers in Grade II Posts</Option>
...
export default GradeIIOfficers;